...
const getFinalAuth = (_authtype: AuthType, _authValues: AuthValues) => {
    switch (_authtype) {
        case "basic":
            return { Authorization: `Basic ${Buffer.from(`${_authValues.username}:${_authValues.password}`).toString("base64")}` }
        case "bearerToken":
            return { Authorization: `Bearer ${_authValues.token}` }
        default:
            return null;
    }
}
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (finalbody === "" && _bodyType === "formData") {
            ...
            const valueKeys = keys.filter((k) => k.includes("formdataValues_"));
            valueKeys.forEach((v) => {
                fd.append(v.split("_")?.[0], formData.get(v)!)
            })
            finalbody = fd;
        }
